...
module.exports = async (options, pkg) => {
	...
	const prompts = [
		...
		},
		{
			type: 'confirm',
			name: 'publishScoped',
			when: isScoped(pkg.name) && !options.availability.isAvailable && !options.availability.isUnknown && options.runPublish && (typeof pkg.publishConfig === 'object' && pkg.publishConfig.access !== 'restricted'),
