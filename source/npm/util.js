...
export const isIgnoreScriptsEnabled = async (rootDirectory, packageManagerCli = 'npm') => {
	try {
		const {stdout} = await execa(packageManagerCli, ['config', 'get', 'ignore-scripts'], {cwd: rootDirectory});
		return stdout.trim() === 'true';
	} catch {
		// If we can't determine the setting, assume scripts run so we don't block a valid publish.
		return false;
	}
};
...
export const verifyPackageEntryPoints = async (package_, rootDirectory, {ignoreScripts = false, ignoreScriptsConfigFile = '.npmrc'} = {}) => {
	...
	if (missingEntryPoints.length > 0) {
		const hasLifecycleScript = hasPackLifecycleScript(package_);

		// A pack lifecycle script (e.g. `prepack`) may generate these files during publish, so we normally trust it. But when `ignore-scripts` is enabled the script won't run, and the package would be published without them.
		if (hasLifecycleScript && !ignoreScripts) {
			return;
		}

		const missing = missingEntryPoints.map(({field, path: entryPath}) => `  "${field}": ${entryPath}`).join('\n');

		// Reaching here with a lifecycle script means `ignoreScripts` is true (otherwise we returned above).
		if (hasLifecycleScript) {
			throw new Error(`Missing entry points in published files:\n${missing}\n\nThese are generated by a pack lifecycle script (e.g. "prepack"), but "ignore-scripts" is enabled in your package manager config, so it won't run during publish and the package would be published without them.\n\nSet the "ignore-scripts" setting to "false" in a project "${ignoreScriptsConfigFile}" to override your global setting.`);
		}
