...
export default function tryEnable2fa(task, packageName, options) {
	return from(enable2fa(packageName, options)).pipe(catchError(error => handleNpmError(error, task, otp => enable2fa(packageName, {otp}))));
}
