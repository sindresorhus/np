	...
	const tasks = [
		{
			...
			task: () => new Promise((resolve, reject) => {
				// `npm ping` waits wayyyy too long
				const timeout = setTimeout(() => {
					reject(new Error('Connection to npm registry timed out'));
				}, 6000);
				execa.stdout('npm', ['ping'])
					.then(() => {
		...
		{
			title: 'Verify user authenticated',
			task: () => new Promise((resolve, reject) => {
				let username;
				// Check if logged in
				execa.stdout('npm', ['whoami'])
					.then(res => {
						username = res;
					}, err => {
						if (/ENEEDAUTH/.test(err.stderr)) {
							reject(new Error('You must be logged in to publish packages. Use `npm login` and try again.'));
						}
						reject(new Error('Authentication error. Use `npm whoami` to troubleshoot.'));
					})
					// Check if has write access to current package
					.then(() => execa.stdout('npm', ['access', 'ls-collaborators', pkg.name]), () => {})
					.then(res => {
						try {
							const json = JSON.parse(res);
							const permissions = json[username];
							if (!permissions || permissions.indexOf('write') === -1) {
								reject(new Error('You do not have write permissions required to publish this package.'));
							}
						} catch (err) {}
						resolve();
					}, () => {
						// The error here is going to be `E404` most of the time, which just means package doesn't exist
						resolve();
					});
			})
		},
