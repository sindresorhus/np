...
module.exports = (input, pkg, opts) => {
	...
	const tasks = [
		...
		{
			title: 'Get git tag prefix using yarn',
			enabled: () => opts.yarn === true,
			task: () => execa.stdout('yarn', ['config', 'get', 'version-tag-prefix'])
				.then(
					output => {
						tagPrefix = output;
					}
		...
		{
			title: 'Get git tag prefix using npm',
			enabled: () => opts.yarn === false,
			task: () => execa.stdout('npm', ['config', 'get', 'tag-version-prefix'])
				.then(
					output => {
						tagPrefix = output;
					}
		...
		},
		{
			...
			task: () => execa('git', ['fetch'])
				.then(() => execa.stdout('git', ['rev-parse', '--quiet', '--verify', `refs/tags/${version.tagPrefixOrDefault(tagPrefix)}${newVersion}`]))
				.then(
					output => {
						if (output) {
							throw new Error(`Git tag \`${version.tagPrefixOrDefault(tagPrefix)}${newVersion}\` already exists.`);
						}
					},
					err => {
