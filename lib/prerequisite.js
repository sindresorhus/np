	...
	const tasks = [
		{
			...
			task: () => pTimeout(execa.stdout('npm', ['ping', ...registryArgs])
				.catch(() => {
		...
		},
		{
			...
			task: () => execa.stdout('npm', ['whoami', ...registryArgs])
				.catch(err => {
				...
				.then(username => execa.stdout('npm', ['access', 'ls-collaborators', pkg.name, ...registryArgs])
					.then(collaborators => {
