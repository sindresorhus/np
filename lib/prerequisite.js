...
module.exports = (input, pkg, opts) => {
	...
	if (opts.npmNetworkChecks) {
		const registryArgs = pkg.publishConfig && pkg.publishConfig.registry ?
		...
		tasks.add([
			{
				...
				task: () => pTimeout(execa.stdout('npm', ['ping'].concat(registryArgs))
					.catch(() => {
						throw new Error('Connection to npm registry failed');
					}), 6000, 'Connection to npm registry timed out')
			},
			{
				title: 'Verify user is authenticated',
				skip: () => process.env.NODE_ENV === 'test',
				task: () => execa.stdout('npm', ['whoami'].concat(registryArgs))
					.catch(err => {
					...
					.then(username => execa.stdout('npm', ['access', 'ls-collaborators', pkg.name].concat(registryArgs))
						.then(collaborators => {
