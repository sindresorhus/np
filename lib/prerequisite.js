...
const {connectToNpm, npmUsername, npmCollaborators, tagVersionPrefix} = require('./npm-util');
	...
	const tasks = [
		{
			...
			task: async () => {
				const connected = await connectToNpm();
				if (connected !== true) {
					throw connected;
				}
			}
		},
		{
			...
			task: async () => {
				...
				if (typeof username === 'string') {
					throw new Error(/ENEEDAUTH/.test(username.stderr) ? // eslint-disable-line unicorn/prefer-type-error
						'You must be logged in to publish packages. Use `npm login` and try again.' :
						'Authentication error. Use `npm whoami` to troubleshoot.');
				}

				const collaborators = await npmCollaborators(pkg.name);
				if (collaborators === '404') {
					return;
				}
		...
		},
		{
			...
			task: async () => {
				...
				const tagPrefix = await tagVersionPrefix(options.yarn);
