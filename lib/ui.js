...
module.exports = async (options, pkg) => {
	...
	if (options.releaseNotes) {
		if (pkg.repository.url) {
			const gitLogs = await getCommitLogs();
			let releaseNotes = 'Changes:\n';
			gitLogs.forEach(message => {
				releaseNotes += `\t- ${message}\n`;
			});
			const gitRepoUrl = pkg.repository.url.replace(/^(git\+)/, '').replace(/(.git)$/, '');
			releaseNotes += `\n${gitRepoUrl}/compare/${oldVersion}...${answers.version}`;
			options.releaseNotesData = {
				repoUrl: gitRepoUrl,
				body: releaseNotes,
				tag: answers.version,
				title: `Release Notes: ${answers.version}`
			};
		} else {
