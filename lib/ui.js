...
module.exports = options => {
	...
	console.log(`\nPublish a new version of ${chalk.bold.magenta(pkg.name)} ${chalk.dim(`(current version: ${oldVersion})`)}\n`);
	...
	function prettyVersionDiff(oldVersion, inc) {
		...
		for (let i = 0; i < newVersion.length; i++) {
			if ((newVersion[i] !== oldVersion[i] && !firstVersionChange)) {
				output.push(`${chalk.dim.bold(newVersion[i])}`);
				firstVersionChange = true;
			} else if (newVersion[i].indexOf('-') >= 1) {
				let preVersion = [];
				preVersion = newVersion[i].split('-');
				output.push(`${chalk.dim.bold(`${preVersion[0]}-${preVersion[1]}`)}`);
			} else {
				output.push(chalk.dim(newVersion[i]));
			}
		}
		return output.join(chalk.dim('.'));
	}
